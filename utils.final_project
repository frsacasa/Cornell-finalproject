...
def str_to_time(timestamp,tz=None):
    ...
    try:
        ...
        elif a.tzinfo == None and tz == None:
            result = a
...
def daytime(time,daycycle):
    ...
    sunset = daycycle[str(time.year)][time.strftime('%m-%d')]['sunset']
    sunrise = daycycle[str(time.year)][time.strftime('%m-%d')]['sunrise']
    zoney = daycycle['timezone']
    zoney1 = pytz.timezone(zoney)
    hourstr, minstr = sunset.split(':')
    hourstr1, minstr1 = sunrise.split(':')
    x = datetime.datetime(time.year,time.month,time.day,int(hourstr),int(minstr),time.second,time.microsecond)
    y = datetime.datetime(time.year,time.month,time.day,int(hourstr1),int(minstr1),time.second,time.microsecond)
    d = zoney1.localize(x)
    d2 = zoney1.localize(y)
    z = d.isoformat()
    y1 = d2.isoformat()
    z1 = str_to_time(z)
    y2 = str_to_time(y1)
    time1 = time.isoformat()
    time2 = str_to_time(time1)
