...
def bad_visibility(visibility,minimum):
    ...
    seen = visibility['minimum'] if 'minimum' in visibility else visibility['prevailing']
...
def bad_ceiling(ceiling,minimum):
    ...
    height = 10000000
    for i in ceiling:
        if i['type'] in ['broken','overcast','indefinite ceiling']:
            height = min(height,i['height'])
...
def get_weather_report(takeoff,weather):
    ...
    tkoffiso = takeoff.isoformat()
    if tkoffiso in weather.keys():
        return weather[str(tkoffiso)]
    for k in weather.keys():
        weatherstr = k
        time = utils.str_to_time(weatherstr,takeoff.tzinfo)
        if time < takeoff:
            return weather[k]
...
def get_weather_violation(weather,minimums):
    ...
    """
                      # Implement this function
    try:   
        visi = weather['visibility']
        windy = weather['wind']
        height = weather['sky']    
        x = bad_visibility(visi, minimums[1])
        y = bad_winds(windy,minimums[2],minimums[3])
        z = bad_ceiling(height,minimums[0])
    except:
        return("Unknown")
    vios = []
    if x == True:
        vios.append("Visibility")
    if y == True:
        vios.append("Winds")
    if z == True:
        vios.append("Ceiling")
...
def list_weather_violations(directory):
    ...
    daytimes = utils.read_json(os.path.join(directory, DAYCYCLE))
    ...
    for lesson in lessons[1:]:
       ...
       # Get the takeoff time
        takeoff = utils.str_to_time(lesson[3],daytimes['timezone'])
        landing = utils.str_to_time(lesson[4],daytimes['timezone'])
        allday  = utils.daytime(takeoff,daytimes) and utils.daytime(landing,daytimes)
        
       # Get the pilot credentials
        student  = utils.get_for_id(lesson[0],students)
        category = pilots.get_certification(takeoff,student)

       # Get the pilot minimums
        teacher = lesson[2]
        filevfr = lesson[5] == 'VFR'
        required = pilots.get_minimums(category, lesson[6], teacher != '', filevfr, allday, minimums)
       ...
       # Check for a violation and add to result if so
        if required is None:
           ...
           violation = get_weather_violation(conditions,required)
